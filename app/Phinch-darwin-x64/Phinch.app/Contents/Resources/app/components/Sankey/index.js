...
export default function Sankey(props) {
  const {
    data, width, height, colors, setRef, renderSVG, helpCounter, clickDatum, colorScale, highlightedDatum
  } = props;
  ...
  const hoverListItem = (node, hoverType = 'list') => (e) => {
    ...
    setHoveredListItem({
      position, name, counts: value, totalCounts: depthOneSum, color, hoverType
    });
  };
  ...
  const listItems = sankeyData.nodes.filter(node => node.depth === maxDepth).sort((a, b) => a.y0 - b.y0);
  ...
  const checkLinkVisibility = (_link) => {
    const visibility = _link.target.hasFinalNodeVisible;
  ...
  const svgList = renderSVG ? (
    <g
      transform={`translate(${width - listWidth}, ${listItemHeight * 0.5})`}
      fontSize="0.8em"
    >
